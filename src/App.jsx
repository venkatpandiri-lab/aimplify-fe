...
function FamilyView({ fk, openA, goBack, nav }) { const f = getFamily(fk); const assets = ASSETS.filter((a) => a.family === fk); const depArr = Array.isArray(f.dependsOn) ? f.dependsOn : [f.dependsOn];
  return <div><div style={{ padding: "14px 40px", fontSize: 13 }}><span onClick={goBack} style={{ color: "#0EA5E9", cursor: "pointer", fontWeight: 500 }}>← Home</span><span style={{ color: "#D1D5DB" }}> / </span><span style={{ fontWeight: 500, color: f.color }}>{f.name}</span></div>
    <div style={{ padding: "0 40px 20px" }}><div style={{ background: f.bg, border: `1px solid ${f.color}25`, borderRadius: 14, borderTop: `4px solid ${f.color}`, display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 0, overflow: "hidden", alignItems: "start" }}>
      <div style={{ padding: "28px 28px 28px 32px", borderRight: `1px solid ${f.color}15` }}><div style={{ fontSize: 11, fontWeight: 700, color: f.color, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>{f.tagline}</div><h1 style={{ fontSize: 26, fontWeight: 700, marginBottom: 12, letterSpacing: "-0.02em", lineHeight: 1.2 }}>{f.name}</h1><p style={{ fontSize: 13, color: "#4B5563", lineHeight: 1.7 }}>{f.longDesc}</p><div style={{ display: "flex", gap: 8, marginTop: 20, flexWrap: "wrap" }}>{[[assets.length, "Assets"], [assets.filter((a) => a.maturity === "battle-tested").length, "Battle-Tested"], [assets.filter((a) => a.demoReady).length, "Demo-Ready"], [assets.reduce((s, a) => s + a.stats.deployments, 0), "Deploys"]].map(([n, l], i) => <div key={i} style={{ background: "#fff", borderRadius: 8, padding: "9px 14px", border: "1px solid #E5E7EB", textAlign: "center", minWidth: 60 }}><div style={{ fontSize: 18, fontWeight: 700, color: f.color }}>{n}</div><div style={{ fontSize: 10, color: "#6B7280", marginTop: 1, fontWeight: 500 }}>{l}</div></div>)}</div></div>
...
function DV({ a, goBack, openA }) { const [tab, setTab] = useState("overview"); const fm = getFamily(a.family); const rel = ASSETS.filter((x) => x.id !== a.id && x.family === a.family).slice(0, 3);
  return <div><div style={{ padding: "14px 40px", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}><span onClick={goBack} style={{ color: "#0EA5E9", cursor: "pointer", fontWeight: 500 }}>← Catalog</span><span style={{ color: "#D1D5DB" }}>/</span><span style={{ color: fm.color, fontWeight: 500 }}>{fm.name}</span><span style={{ color: "#D1D5DB" }}>/</span><span style={{ fontWeight: 500 }}>{a.name}</span></div>
    <div style={{ padding: "0 40px 48px", display: "grid", gridTemplateColumns: "1fr minmax(260px, 320px)", gap: 32 }}><div style={{ display: "flex", flexDirection: "column", gap: 20 }}><div><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" }}><span style={{ fontFamily: "'JetBrains Mono'", fontSize: 11, color: "#9CA3AF", background: "#F3F4F6", padding: "3px 9px", borderRadius: 4, border: "1px solid #E5E7EB" }}>{a.id}</span><Bdg color={fm.color} label={fm.name} /><Bdg color={MC[a.maturity]} label={ML[a.maturity]} />{a.demoReady && <Bdg color="#22C55E" label="Demo Ready" />}</div><h1 style={{ fontSize: 24, fontWeight: 700, lineHeight: 1.2 }}>{a.name}</h1><div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>{a.clouds.map((c) => <span key={c} style={{ fontSize: 11, padding: "4px 10px", borderRadius: 8, background: `${CC[c]}0A`, color: CC[c], border: `1px solid ${CC[c]}20`, fontWeight: 500 }}>{CL[c]}</span>)}</div><div style={{ marginTop: 8, fontSize: 12, color: fm.color, fontWeight: 500 }}>{a.solution}</div><p style={{ fontSize: 14, color: "#6B7280", lineHeight: 1.6, marginTop: 8, maxWidth: 580 }}>{a.desc}</p><div style={{ display: "flex", gap: 10, marginTop: 14 }}><Btn primary label="Launch Demo" disabled={!a.demoReady} /><Btn label="Clone Repo" /></div></div>
      ...
      <div><Crd><CrdT>About</CrdT>{a.longDesc.split("\n\n").map((p, i) => <p key={i} style={{ fontSize: 13, color: "#4B5563", lineHeight: 1.7, marginTop: i ? 10 : 0 }}>{p}</p>)}<div style={{ display: "flex", gap: 5, marginTop: 14, flexWrap: "wrap" }}>{a.tags.map((t) => <span key={t} style={{ fontSize: 10, padding: "3px 9px", borderRadius: 14, background: "#F3F4F6", color: "#6B7280", fontWeight: 500 }}>{t}</span>)}</div></Crd>
      ...
      <aside style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 0 }}><SC t="Usage"><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}><SB n={a.stats.deployments} l="Deploys" c="#0EA5E9" /><SB n={a.stats.demos} l="Demos" c="#22C55E" /><SB n={a.stats.projects} l="Projects" c="#F59E0B" /><SB n={`${a.stats.satisfaction}%`} l="Rating" c="#8B5CF6" /></div></SC>
        <SC t="Details">{[["ID", a.id], ["Family", fm.name], ["Category", a.category], ["Solution", a.solution], ["Maturity", ML[a.maturity]], ["Effort", a.effort.charAt(0).toUpperCase() + a.effort.slice(1)]].map(([k, v]) => <div key={k} style={{ display: "grid", gridTemplateColumns: "76px 1fr", columnGap: 8, alignItems: "start", padding: "6px 0", borderBottom: "1px solid #F3F4F6", fontSize: 12 }}><span style={{ color: "#9CA3AF", lineHeight: 1.45 }}>{k}</span><span style={{ fontWeight: 500, color: k === "Family" ? fm.color : k === "Maturity" ? MC[a.maturity] : "#1a1a2e", fontFamily: k === "ID" ? "'JetBrains Mono'" : "inherit", fontSize: k === "ID" ? 11 : 12, lineHeight: 1.45, minWidth: 0, wordBreak: "break-word", overflowWrap: "break-word" }}>{v}</span></div>)}</SC>
        <SC t="Owner"><div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "#F9FAFB", borderRadius: 10 }}><div style={{ width: 34, height: 34, borderRadius: "50%", background: `linear-gradient(135deg,${fm.color},#8B5CF6)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 11, fontWeight: 600, flexShrink: 0 }}>{a.ownerInit}</div><div style={{ fontSize: 13, fontWeight: 600 }}>{a.owner}</div></div></SC>
...
function SC({ t, children }) { return <div style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 12, padding: "14px 16px", minWidth: 0 }}><div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", color: "#9CA3AF", fontWeight: 600, marginBottom: 10 }}>{t}</div>{children}</div>; }
